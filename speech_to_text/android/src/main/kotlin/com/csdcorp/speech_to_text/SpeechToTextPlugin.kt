...
public class SpeechToTextPlugin :
    ...
    override fun onAttachedToActivity(binding: ActivityPluginBinding) {
        ...
        binding.addRequestPermissionsResultListener(this)
        audioManager =
            binding.activity.getApplicationContext()
    ...
    private fun mute(shouldMute: Boolean) {
        ...
        if (manager != null) {
            Log.i("SpeechRecognition", "Mute: $shouldMute")

            val flag = if (shouldMute) AudioManager.ADJUST_MUTE else AudioManager.ADJUST_UNMUTE;
            manager.adjustStreamVolume(AudioManager.STREAM_MUSIC, flag, 0);
//
//            manager.setStreamMute(AudioManager.STREAM_MUSIC, shouldMute)
//            if (shouldMute) {
//                previousVolume = manager.getStreamVolume(AudioManager.STREAM_MUSIC);
//                Log.i("SpeechRecognition", "Previous volume: $previousVolume")
//            } else {
...
//            }
    ...
    private fun initializeIfPermitted(context: Context?) {
        ...
        val permissionToEnableBluetooth = ContextCompat.checkSelfPermission(
            localContext,
